...
//				wowzaScheduleResponse = wowzaClient.executePost("schedules", liveStream.getCloudJSONObjectForSchedule());
		...
		JSONObject jObj = new JSONObject();
		jObj.put(IConstants.EVENT, eventHelper.buildResponseObject(eventEntity, null, true));
...
//			}
		...
		JSONObject jObj = new JSONObject();
		jObj.put(IConstants.EVENT, eventHelper.buildResponseObject(event.get(), (viewerEntity != null && viewerEntity.isPresent()) ? viewerEntity.get() : null, true));
		result = utils.constructSucessJSON(jObj);
		return result;
	}
...
//			}
	...
	public String getEventByName(String eventName, BroadcasterInfo broadcasterInfo) throws ResourceNotFoundException, BusinessException, Exception {
		...
		JSONObject jObj = new JSONObject();
		jObj.put(IConstants.EVENT, eventHelper.buildResponseObject(eventList, null));
		result = utils.constructSucessJSON(jObj);
		return result;
	}
...
//		}
		...
		JSONObject jObj = new JSONObject();
		jObj.put(IConstants.EVENTS, eventHelper.buildResponseObject(eventList, (viewerEntity != null && viewerEntity.isPresent()) ? viewerEntity.get() : null));
