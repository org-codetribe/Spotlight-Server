...
public interface IEventRepository extends CrudRepository<Event, Integer> {
   ...
   // @Query(value = "select * from event e where e.event_UTC_Datetime >= (sysdate() + e.event_duration + 1/24) order by event_UTC_Datetime desc",nativeQuery = true)
    @Query(value = "select * from event e",nativeQuery = true)
    ...
    @Query(value = "select * from event where LOWER(display_name) LIKE lower(concat('%', :name,'%'))",nativeQuery = true)
   ...
   // @Query(value = "SELECT info.id,COUNT(event.broadcaster_info_id) AS event_count FROM spotlight.event INNER JOIN spotlight.broadcaster_info info where event.broadcaster_info_id = info.id and event_UTC_Datetime >= (sysdate()+event.event_duration + 1/24) group by event.broadcaster_info_id order by event_count desc",nativeQuery = true)
    @Query(value = "SELECT info.id,COUNT(event.broadcaster_info_id) AS event_count FROM spotlight.event INNER JOIN spotlight.broadcaster_info info where event.broadcaster_info_id = info.id  group by event.broadcaster_info_id order by event_count desc",nativeQuery = true)
