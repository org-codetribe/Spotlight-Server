...
public class ViewerRestController {
    ...
    public @ResponseBody
    String registerViewer(@RequestBody String requestBody,
    ...
    @RequestMapping(value = "/api/1.0/register/viewer", method = RequestMethod.PATCH, consumes = {
            MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public @ResponseBody
    String registerViewerWithSocial(@RequestBody String requestBody,
        ...
        try {
            if (viewer.getSocialLoginType().equalsIgnoreCase("FB")) {
                ...
                if (byEmailFB != null) {
                    if (byEmailFB.getEmail() == null || byEmailFB.getEmail() == "" || byEmailFB.getEmail().length() == 0) {
                    ...
                    if (viewer.getEmail() == null || viewer.getEmail() == "" || viewer.getEmail().length() == 0) {
                        JSONObject jObj = new JSONObject();
                        jObj.put(IConstants.SUCCESS, false);
    ...
    @RequestMapping(value = "${jwt.route.authentication.viewer.path}", method = RequestMethod.POST, consumes = {
            MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public @ResponseBody
    String createAuthenticationToken(@RequestBody String requestBody,
    ...
    public @ResponseBody
    String getAuthenticatedUser(@RequestHeader("Authorization") String authorization)
    ...
    @RequestMapping(value = "api/1.0/resetViewerPassword", method = RequestMethod.PUT, consumes = {
            MediaType.APPLICATION_JSON_VALUE}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public @ResponseBody
    String resetViewerPassword(@RequestBody String requestBody,
    ...
    @RequestMapping(value = "api/1.0/logout/viewer", method = RequestMethod.DELETE, produces = {
            MediaType.APPLICATION_JSON_VALUE})
    public @ResponseBody
    String logoutViewer(@RequestHeader("Authorization") String authorization)
            throws SpotlightAuthenticationException, BusinessException {
...
//		String username = jwtTokenUtil.getUsernameFromToken(authorization.substring(7));
    ...
    @RequestMapping(value = "api/1.0/email-exist/viewer", method = RequestMethod.GET, produces = {
            MediaType.APPLICATION_JSON_VALUE})
    public @ResponseBody
    String emailExist(@RequestParam("email") String email)
            throws SpotlightAuthenticationException, BusinessException {
